...
client.on("message", async message => {
  ...
  if (message.content.match(prefixMention)) {
    return message.reply(`Prefix gw \`${default_prefix}\``);
  }
...
var channel = client.channels.cache.get('812592647876247579');
 ...
 client.once('ready', () => {
    ...
    setInterval(() => {
        const statuses = [
            `${client.users.cache.size} : Beban Keluarga`,
            `You 😳 `,
            `Ketik ~Help`,
        ]
    ...
    }, 6000) // Second You Want to Change Status, This Cahnges Every 2 Seconds
}) 
